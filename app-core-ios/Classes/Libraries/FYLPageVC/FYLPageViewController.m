...
-(void)setItems:(NSArray *)items viewControllers:(NSArray *)viewControllers{
    ...
    if (!_viewControllers.count || !_items.count) {
        return;
    }
...
- (void)scrollToNextCompletion:(void(^)(NSInteger index))handler{
    if (_currentPage >= (self.viewControllers.count-1)) {
        return;
    }
    _currentPage++;
    [self.pageVC setViewControllers:@[self.viewControllers[_currentPage]] direction:NO animated:YES completion:^(BOOL finished) {
        if (handler) {
            handler(_currentPage);
        }
    }];
}
...
- (void)scrollToPreviousCompletion:(void(^)(NSInteger index))handler{
    ...
    _currentPage--;
    [self.pageVC setViewControllers:@[self.viewControllers[_currentPage]] direction:YES animated:YES completion:^(BOOL finished) {
        if (handler) {
            handler(_currentPage);
        }
    }];
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray<UIViewController *> *)previousViewControllers transitionCompleted:(BOOL)completed {
    ...
    if ([_delegate respondsToSelector:@selector(scrollNextViewWithIndex:)]) {
        [_delegate scrollNextViewWithIndex:index];
    }
}
...
- (FYLSegmentView *)viewSegment{
    ...
    if (!_viewSegment) {
        _viewSegment = [[FYLSegmentView alloc] initWithItems:self.items];
